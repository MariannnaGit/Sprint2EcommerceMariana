...
// Exercise 4
function applyPromotionsCart(cartTotalPrint) {
    // Apply promotions to each item in the array "cart"
    let subtotalWithDiscount = cart.map((prod) => {
        if (prod.quantity > 3 && prod.id == 1) {
            total = (prod.quantity * prod.price) * 20/100;
            console.log(cartTotal)
        }
        if (prod.quantity >= 10) {
            total = (prod.quantity * prod.price) * 30/100;
            console.log(cartTotal)
        } else {
            total = cartTotalPrint
        }
    })
...
function printCart() {
    ...
    applyPromotionsCart(cartTotalPrint) 
